...
namespace WindowsServerManager.Components.Libraries.Docker
{
    public static class DockerStatsFormatter
    {
        ...
        private static string CalculateCpuPercentage(StatApiReturn? jsonData)
        {
            try
            {
                long curTotalUsage = jsonData?.CpuStats?.CpuUsage?.TotalUsage ?? 0;
                long preTotalUsage = jsonData?.PrecpuStats?.CpuUsage?.TotalUsage ?? 0;

                long curSystemUsage = jsonData?.CpuStats?.SystemCpuUsage ?? 0;
                long preSystemUsage = jsonData?.PrecpuStats?.SystemCpuUsage ?? 0;

                int opus = jsonData?.CpuStats?.OnlineCpus ?? 0;
        ...
        private static string CalculateMemoryPercentage(StatApiReturn? jsonData)
        {
            try
            {
                int usedMemory = jsonData?.MemoryStats?.Usage ?? 0;
                long memoryLimit = jsonData?.MemoryStats?.Limit ?? 0;
