...
namespace WindowsServerManager.Libraries.NotificationClients
{
    ...
    public class NotificationClient
    {
        ...
        public NotificationClient(Settings settings)
        {
            ...
            try
            {
                _notifications = JsonSerializer.Deserialize<List<Notification>>(File.ReadAllText("notifications.json")) ?? [];
            }
            catch
            {
                _notifications = [];
        ...
        private async Task SendNotification(Notification notification)
        {
            ...
            if (discordSettings is not null && (discordSettings?.WebhookEnabled ?? false))
                await Task.Run(async () => await DiscordWebhookService.Send(note.Text, note.Title, note.Severity, discordSettings?.RolePing ?? "", note.Timestamp, discordSettings?.WebhookUrl!));
        ...
        private void DisplayNotification(Notification notification, ref ISnackbar snackBar)
        {
            Notification? note = _notifications.FirstOrDefault(x => x.Id == notification.Id);
            if (note is null) return;
        ...
        private void WriteDatabase() => File.WriteAllText("notifications.json", JsonSerializer.Serialize(_notifications));
    }
}
