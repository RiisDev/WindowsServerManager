...
namespace WindowsServerManager.SystemUpdater
{
    public class LogService
    {
        public LogService(string logName)
        {
            ...
            try
            {
                if (!EventLog.SourceExists("Windows Server Manager - SysUpd"))
                {
                    EventLog.CreateEventSource("Windows Server Manager - SysUpd", "Application");
                }
            }
            catch {/**/}
        }
        ...
        private void LogToEventLog(string? message, EventLogEntryType entryType)
        {
            try
            {
                ...
                eventLog.Source = "Windows Server Manager - SysUpd";
