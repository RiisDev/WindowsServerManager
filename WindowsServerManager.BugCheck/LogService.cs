...
namespace WindowsServerManager.BugCheck
{
    public class LogService
    {
        public LogService(string logName)
        {
            ...
            try
            {
                if (!EventLog.SourceExists("Windows Server Manager - Bug Check"))
                {
                    EventLog.CreateEventSource("Windows Server Manager - Bug Check", "Application");
                }
            }
            catch {/**/}
        }
        ...
        private void LogToEventLog(string? message, EventLogEntryType entryType)
        {
            try
            {
                ...
                eventLog.Source = "Windows Server Manager - Bug Check";
